...
* **Start Phase Tag**: Will be the phase of when the ability gets commited (Not Activated!). Also when a input gets added to a queue, input will find the corresponding ability's start phase tag's priority in order to make Ability Queue System to determine if the input ability should be activated or not. So you can think Start Phase tag as "input's priority".
...
:::note
Your ability phases doesn't need to be strict except "start phase tag" and "end phase tag", they can be defined per action. For example: Drawing bow could have a "Start, TakeBow, DrawBow, Loop, ReleaseArrow, Recovery, End" phases. Because Ability Queue System only tracks the "CurrentPhase's Priority" it doesn' t need to know what is the exact phase. This way in the event from Queueable Ability; **OnPhaseChanged** you can get the new phase and perform new actions, events etc. 
